...
export default function SubmitButton({ wordInput }: SearchInputProps) {
  ...
  return (
    <button
      type="button"
      className="h-full w-auto flex justify-center items-center bg-(--submit-button) border-none rounded-r-md px-3 transition-opacity duration-200 cursor-pointer flex-shrink-0"
      onClick={() => performSearch(wordInput)}
      aria-label="検索実行"
    >
      <svg
        width="16"
