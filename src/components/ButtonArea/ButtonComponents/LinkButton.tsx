...
export const LinkButton: React.FC<DomainItem> = ({
  ...
  return (
    <button
      onClick={handleClick}
      className="inline-flex items-center justify-center border-1 border-(--button-border) w-auto min-w-18 h-[2.7em] bg-(--button-color) text-(--button-text-color) no-underline py-2 px-3.5 rounded-md flex-shrink-0 transition-colors duration-200 cursor-pointer hover:text-(--button-text-color-hover)"
      type="button"
    >
      {name}
    </button>
